...
class testDataset(Dataset):
    def __init__(self, dataset_name="XUANWU", i=0, using_ictal=0, patient_id=None, patient_name=None, ch_num=1, target_preictal_interval=15, step_preictal=5, is_test = True, data_dir = None):
        data, label = [], []
        ...
        # if self.is_test and dataset_name=="CHB":
        ...
        print("preIctal {} | interIctal {}".format(self.preictal_length, self.interictal_length))
...
class testDataset_wzx(Dataset):
    def __init__(self, dataset_name="XUANWU", i=0, using_ictal=0, patient_id=None, patient_name=None, ch_num=1, target_preictal_interval=15, step_preictal=5, is_test = True, data_dir = None):
        data, label = [], []
        self.data_path=GetDataPath(dataset_name) #data path
        if dataset_name == 'KAGGLE' and data_dir == None:
            self.data_path = '/mnt/share/zjc/Kaggle_wzx/Kaggle'
        elif dataset_name == 'CHB' and data_dir == None:
            self.data_path = '/mnt/share/zjc/CHBMIT_new_split'
        else:
            self.data_path = data_dir
        ...
        #data loading
        self.target_preictal_interval = 30
        preIctal = np.load(("%s/%s/%dmin_%dstep_%dch/preictal%d.npy" % (self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
        interIctal = np.load(("%s/%s/%dmin_%dstep_%dch/interictal%d.npy" % (self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
        # if self.is_test and dataset_name=="CHB":
        #     Ictal = np.load(("%s/%s/%dmin_%dstep_%dch/ictal%d.npy" % (self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
