...
class trainDataset(Dataset):
    def __init__(self, model_name="CNN", dataset_name="CHB", n=[], ite=1, augmentation=1, using_ictal=1,
                 scaler=0, patient_id=None, patient_name=None, ch_num=1, target_preictal_interval=15, step_preictal=5, data_dir=None):
        data, label = [], []
        ...
        for i in n:
            ...
            if dataset_name=="CHB" and (model_name.startswith("CNN") or model_name.startswith("VIT") or model_name.startswith("ResNet")):
                # print("-----------CNN-----------VIT------------ResNet----------------")
                # print("model_name========", model_name)
                # if model_name.startswith("CNN") or model_name.startswith("VIT"):
                #     print("CHB----------------CNN")
                ...
                print("----------CHB----------------------")
                if self.patient_id in (8, 14, 16, 23): #interictal比preictal还少，因此不适用2倍增强后的数据
                    preIctal = np.load(("%s/%s/%dmin_%dstep_%dch/preictal%d.npy" % (
                    self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
                else:
                    preIctal = np.load(("%s/%s/%dmin_%dstep_%dch_gauss8/preictal%d.npy" % (
                    self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
                if self.patient_id in (2, 7, 9, 11, 21, 22):  # (1, 8, 14, 16, 20, 23) #这部分患者没有30min_5step_18ch_gauss8文件夹中的interictal数据，所以读原始不加噪声的interictal数据
                    interIctal = np.load(("%s/%s/%dmin_%dstep_%dch/interictal%d.npy" % (
                    self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
                else:
                ...
                # # self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
            elif dataset_name=="Kaggle" and model_name=="swin_AlignFA_Third_inerGuass_pre30min":
                preIctal = np.load(("%s/%s/%dmin_%dstep_%dch/preictal%d.npy" % (
                    self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
                interIctal = np.load(("%s/%s/%dmin_%dstep_%dch_gauss8/interictal%d.npy" % (
                    self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
            ...
            else:   # dataset_name=="Kaggle":
                preIctal = np.load(("%s/%s/%dmin_%dstep_%dch/preictal%d.npy" % (
                    self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
                interIctal = np.load(("%s/%s/%dmin_%dstep_%dch/interictal%d.npy" % (
                    self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
                interIctal = interIctal[len(interIctal)//2:,:,:]
                # if self.patient_id in (3, 4):
                #     preIctal = np.load(("%s/%s/%dmin_%dstep_%dch/preictal%d.npy" % (
                #     self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
                # # elif self.patient_id == 5:
                # #     preIctal = np.load(("%s/%s/%dmin_%dstep_%dch_60minpre/preictal%d.npy" % (
                # #     self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
                # else:
                #     preIctal = np.load(("%s/%s/%dmin_%dstep_%dch_gauss8/preictal%d.npy" % (
                #     self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
...
class ThirdDataset(Dataset):
    def __init__(self, dataset_name="CHB", n=[], ite=1, augmentation=1, using_ictal=1,
        ...
        for i in n:
            # data loading from npy files
            thirdClass = np.load(("%s/%s/%dmin_%dstep_%dch_interictal_gauss/interictal%d.npy" % (
            self.data_path, self.patient_name, self.target_preictal_interval, self.step_preictal, self.ch_num, i)))
